...
// Additional service worker prevention (redundant but safe)
...
if (typeof acquireVsCodeApi !== "undefined") {
  ...
  console.error(
    "acquireVsCodeApi is not available. Ensure this script is running in a VS Code Webview context.",
  );
}

// Enhanced error handling for webview context
...
window.addEventListener("unhandledrejection", function (event) {
  ...
  // Suppress service worker related promise rejections
  if (
    event.reason &&
    ((typeof event.reason === "string" && event.reason.includes("ServiceWorker")) ||
      (event.reason.message && event.reason.message.includes("ServiceWorker")))
  ) {
    event.preventDefault();
    console.log("Service worker promise rejection suppressed");
    return false;
  }
});

// Log successful initialization
console.log("Theme Lab webview script loaded, service worker prevention active");
...
function pushHistory() {
  undoStack.push(clone(theme));
  redoStack.length = 0;
  updateUndoRedoButtons();
}
function undo() {
  if (!undoStack.length) return;
  redoStack.push(clone(theme));
  theme = undoStack.pop();
  renderAll();
  postThemeChanged();
  updateUndoRedoButtons();
...
// Safely post messages to VS Code extension host
function post(type, payload = {}) {
  if (vscode && typeof vscode.postMessage === "function") {
    vscode.postMessage({ type, ...payload });
  } else {
    console.warn("vscode API not available; message not sent:", { type, ...payload });
  }
}
function postUiState() {
  post("uiStateChanged", { ui: { liveEnabled, filter: el("#filter").value.trim(), selectedKey } });
}
function postThemeChanged() {
  post("themeChanged", { theme });
}
function setLiveEnabled(v) {
  liveEnabled = !!v;
  post("toggleLive", { value: liveEnabled });
  postUiState();
}
...
function renderCategoriesTree() {
  ...
  function addNode(node, container) {
    ...
    const nKeys =
      (node.keys?.length || 0) +
      (node.children?.reduce((s, c) => s + (c.keys?.length || 0), 0) || 0);
    ...
    (node.keys || []).forEach((k) => {
      ...
      chip.style.background = theme.colors[k] || "transparent";
  ...
  el("#expandAll").onclick = () => {
    els("#catBody .children").forEach((c) => (c.style.display = "block"));
    els("#catBody .chev").forEach((c) => (c.textContent = "▾"));
  };
  el("#collapseAll").onclick = () => {
    els("#catBody .children").forEach((c) => (c.style.display = "none"));
    els("#catBody .chev").forEach((c) => (c.textContent = "▸"));
  };
...
function renderColors() {
  const keys = Object.keys(theme.colors).length
  ...
  filteredKeys.forEach((k) => {
    ...
    rowVal.type = "text";
    rowVal.className = "color-input tooltip";
    rowVal.placeholder = "Enter color: #rrggbb, #rrggbbaa, or rgba(...)";
    rowVal.setAttribute(
      "data-tooltip",
      "Supported formats: #rrggbb (RGB), #rrggbbaa (RGBA), rgba(r,g,b,a), or color names",
    );
    ...
    rowVal.addEventListener("change", () => {
      ...
      updatePreviewStyleForKey(k);
      renderDiff(k);
      els(".key-item .key-name")
        .filter((n) => n.textContent === k)
        .map((n) => n.previousSibling)
        .forEach((ch) => (ch.style.background = theme.colors[k] || "transparent"));
      postThemeChanged();
    });
    const pick = document.createElement("button");
    pick.className = "pick-btn tooltip";
    pick.textContent = "🎨";
    ...
    if (descriptions[k]) {
      ...
      d.className = "key-desc";
      d.textContent = descriptions[k];
...
function renderSemantic() {
  ...
  Object.keys(theme.semanticTokenColors).forEach((k) => {
    ...
    color.addEventListener("change", () => {
      ...
      theme.semanticTokenColors[k] =
        typeof val === "string" ? v : { ...(val || {}), foreground: v };
...
const previewMap = {
  ...
  "list.activeSelectionBackground": (v) =>
    document.querySelectorAll(".explorer li.active").forEach((li) => {
...
function setVar(name, val) {
  document.documentElement.style.setProperty(name, normalizeColor(val));
}
function applyPreviewStyles() {
  Object.keys(theme.colors || {}).forEach(updatePreviewStyleForKey);
  applyTokenPreview();
}
function updatePreviewStyleForKey(key) {
  const fn = previewMap[key];
  if (fn) fn(theme.colors[key]);
  if (key === "button.background")
    els(".demo-btn").forEach((b) => (b.style.background = normalizeColor(theme.colors[key])));
  if (key === "input.background")
    els(".demo-input").forEach((i) => (i.style.background = normalizeColor(theme.colors[key])));
  if (key === "input.foreground")
    els(".demo-input").forEach((i) => (i.style.color = normalizeColor(theme.colors[key])));
  if (key === "input.border")
    els(".demo-input").forEach((i) => (i.style.borderColor = normalizeColor(theme.colors[key])));
}
  ...
  for (const rule of theme.tokenColors || []) {
    ...
    const scopes = Array.isArray(rule.scope)
      ...
      : String(rule.scope)
          .split(",")
          .map((s) => s.trim());
    if (scopes.some((s) => scopesList.some((want) => s.includes(want))))
...
function applyTokenPreview() {
  ...
  for (const [k, v] of Object.entries(map)) {
    if (v) document.documentElement.style.setProperty(k, normalizeColor(v));
  }
}
...
function renderDiff(key) {
  ...
  cont.innerHTML = "";
  if (!key) return;
  const old = theme.colors[key] || "";
  const row = document.createElement("div");
  row.className = "row";
  const box = document.createElement("span");
  box.className = "color-box";
  box.style.background = normalizeColor(old) || "transparent";
  const lab = document.createElement("span");
  lab.textContent = `${key}: ${old || "(empty)"}`;
  row.appendChild(box);
  row.appendChild(lab);
...
(function makeResizable() {
  ...
  g2.className = "gutter";
  cols.insertBefore(g1, cols.children[1]);
  cols.insertBefore(g2, cols.children[3]);
  let drag = null;
  function start(which, ev) {
    drag = { which, x: ev.clientX, sizes: getSizes() };
    ev.preventDefault();
  }
  function getSizes() {
    const cs = getComputedStyle(cols);
    return cs.gridTemplateColumns.split(" ").map((x) => x.trim());
  }
  function setSizes(arr) {
    cols.style.gridTemplateColumns = arr.join(" ");
  }
  ...
  window.addEventListener("mousemove", (ev) => {
    ...
    if (drag.which === 1) {
      const a = parseInt(sizes[0]);
      sizes[0] = Math.max(220, a + dx) + "px";
    } else {
      const e = parseInt(sizes[2]);
      sizes[2] = Math.max(420, e + dx) + "px";
    }
    setSizes(sizes);
  });
...
const overlay = el("#pickerOverlay"),
  pickerBase = el("#pickerBase"),
  pickerHex = el("#pickerHex"),
  pickerAlpha = el("#pickerAlpha"),
  pickerAlphaVal = el("#pickerAlphaVal"),
  swatchesRoot = el("#swatches"),
  pickerApply = el("#pickerApply"),
  pickerCancel = el("#pickerCancel");
...
function openPickerColor(key, inputEl) {
  openPickerCommon(theme.colors[key] || "#ccccccff");
  pickerTarget = { kind: "color", key, inputEl };
}
function openPickerToken(index, inputEl) {
  const v = theme.tokenColors[index]?.settings?.foreground || "#ccccccff";
  openPickerCommon(v);
  pickerTarget = { kind: "token", index, inputEl };
}
function openPickerSemantic(key, inputEl) {
  const cur = theme.semanticTokenColors[key];
  const v = typeof cur === "string" ? cur : cur?.foreground || "#ccccccff";
  openPickerCommon(v);
  pickerTarget = { kind: "semantic", key, inputEl };
}
function openPicker(key, inputEl) {
  openPickerColor(key, inputEl);
}
...
pickerApply.addEventListener("click", () => {
  ...
  if (pickerTarget.kind === "color") {
    ...
    updatePreviewStyleForKey(pickerTarget.key);
    ...
    theme.semanticTokenColors[k] =
      typeof cur === "string" ? val : { ...(cur || {}), foreground: val };
    pickerTarget.inputEl.value = val;
    post("updateSemantic", { key: k, value: theme.semanticTokenColors[k] });
  }
  postThemeChanged();
...
el("#themeName").addEventListener("change", (e) => {
  pushHistory();
  post("rename", { name: e.target.value });
  theme.name = e.target.value;
  postThemeChanged();
});
el("#themeType").addEventListener("change", (e) => {
  pushHistory();
  post("setType", { value: e.target.value });
...
window.addEventListener("message", (ev) => {
  ...
  if (msg.type === "templateLoaded") {
    ...
    if (msg.persisted?.ui) {
      ...
      el("#filter").value = msg.persisted.ui.filter || "";
    ...
    renderAll();
    if (selectedKey) {
      highlightPreview(selectedKey);
      renderDiff(selectedKey);
    }
    postUiState();
  }
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", function () {
    console.log("Theme Lab webview loaded successfully");
    initializeThemeLab();
  });
} else {
  // DOM is already ready
  console.log("Theme Lab webview loaded successfully (DOM ready)");
  initializeThemeLab();
}

function initializeThemeLab() {
  try {
    renderCategoriesTree();
    renderAll();
    updateUndoRedoButtons();
    console.log("Theme Lab initialized successfully");
  } catch (error) {
    console.error("Error initializing Theme Lab:", error);
    // Try to show a user-friendly error
    document.body.innerHTML =
      '<div style="padding: 20px; color: #ff6b6b;">Error loading Theme Lab. Please try reopening the panel.</div>';
  }
}
