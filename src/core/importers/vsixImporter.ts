 ...
 */
export async function importVsixTheme (vsixPath: string): Promise<ThemeSpec | undefined> {
	...
	const themes = (pkg.contributes?.themes ?? []) as ThemeContribution[];
	if (!themes.length) throw new Error("No themes contributed in VSIX");

	let picked = themes[0];
	if (themes.length > 1) {
		const items: { label: string; description: string; path: string }[] = themes.map((t: ThemeContribution) => ({
		...
		picked = themes.find((t: ThemeContribution) => t.path === pick.path) || themes[0];
	}
